...
*   **`EscaMaker`**: O projeto principal Blazor WebAssembly, contendo a lógica da interface do usuário (UI), serviços do lado do cliente e utilitários.
    ...
    *   `Services`: Implementações de lógica de negócios e abstrações para interações externas (ex: `PDFEscala` para geração de PDF da escala completa, `EscalaPessoaPdf` para PDFs de escalas individuais, `JsonExport` para manipulação de JSON, e `AdminApiService` para interações com o backend).
...
O projeto EscaMaker encontra-se em desenvolvimento ativo com foco em melhorias de experiência do usuário (UX/UI) e funcionalidades de autenticação.

### Implementado

*   ✅ Autenticação de usuário com sistema de tokens (`adminToken`)
*   ✅ Página de login (`Login.razor`) com autenticação via `AdminApiService`
*   ✅ Persistência de dados em `localStorage` para manutenção de sessão
*   ✅ Integração com API backend (`AdminApiService`)
*   ✅ Geração de escalas por ano e mês
*   ✅ Exportação e importação de dados em JSON
*   ✅ Geração de documentos PDF (escalas completas e individuais)
*   ✅ Sistema de consultas por pessoa e período
*   ✅ Componentes MudBlazor para interface moderna

### Corrigido / Melhorado

*   ✅ **UX/UI da Página de Login**: Implementado indicador visual de carregamento (`MudProgressCircular`) durante o processo de autenticação
*   ✅ **Ocultar Formulário During Loading**: O formulário é ocultado enquanto o login está em processamento, exibindo um spinner com mensagem "Authenticating..."
*   ✅ **Design Consistency**: Formulário envolvido em `MudPaper` para manter consistência visual com a página de sessão ativa
*   ✅ **Campo Icons**: Adicionados ícones aos campos de username e password para melhor identificação
*   ✅ **Variant Update**: Campos de texto alterados para variante `Outlined` para aparência mais moderna
*   ✅ **Button Layout**: Botão de login agora ocupa a largura total (`FullWidth`) para melhor usabilidade em dispositivos móveis
*   ✅ **Alert Styling**: Alertas de erro com variante `Filled` para maior destaque
